...
class Env(object):
    ...
    def if_done(self, state):
        ...
        if (-120.0 > self.rad2ang(self.angles[self.arm0 + 0]) or self.rad2ang(self.angles[self.arm0 + 0]) > 120.0
            ...
            return [True, "Limit"]
        # precision
        elif np.linalg.norm(self.target - state.numpy()) < PRECISION:
            return [True, "Target"]
        else:
            return [False, None]
    ...
    def get_reward(self, s):
        ...
        if self.if_done(s)[1] == 'Limit':
            ...
            r = 1 / np.linalg.norm(self.target - s.numpy()) * 1000
        return r
    ...
    def get_state(self):
        ...
        return torch.tensor([pos.x, pos.y, pos.z]).type(torch.FloatTensor)
